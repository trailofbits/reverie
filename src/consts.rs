pub const BATCH_SIZE: usize = 10_000_000;

pub const CONTEXT_RNG_BEAVER: &str = "beaver_seed";

pub const CONTEXT_RNG_INPUT_MASK: &str = "input_mask_seed";

pub const CONTEXT_RNG_BRANCH_MASK: &str = "branch_mask_seed";

pub const CONTEXT_RNG_BRANCH_PERMUTE: &str = "branch_permute_seed";

pub const CONTEXT_RNG_CORRECTION: &str = "correction_seed";

pub const CONTEXT_ORACLE_ONLINE: &str = "oracle_online";

pub const CONTEXT_ORACLE_PREPROCESSING: &str = "oracle_preprocessing";

pub const BYTES_PER_GATE: i64 = 300;

pub const GATE_MEM_INTERCEPT: i64 = 38_400_000;
