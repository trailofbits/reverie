...
use crate::consts::{CONTEXT_RNG_EDA, CONTEXT_RNG_EDA_2, CONTEXT_RNG_INPUT_2_MASK, CONTEXT_RNG_INPUT_MASK};
use crate::crypto::*;

pub struct SharesGenerator<D: Domain, D2: Domain> {
    pub input: ShareGenerator<D>,
    pub input_2: ShareGenerator<D2>,
...
impl<D: Domain, D2: Domain> SharesGenerator<D, D2> {
    pub fn new(player_seeds: &[[u8; KEY_SIZE]]) -> Self {
        let input_prgs: Vec<PRG> = player_seeds
            ...
            .collect();
        let input_2_prgs: Vec<PRG> = player_seeds
            .iter()
            .map(|seed| PRG::new(kdf(CONTEXT_RNG_INPUT_2_MASK, seed)))
            .collect();
        ...
        Self {
            ...
            input_2: ShareGenerator::new(input_2_prgs),
...
pub struct PartialSharesGenerator<D: Domain, D2: Domain> {
    ...
    pub input_2: PartialShareGenerator<D2>,
...
impl<D: Domain, D2: Domain> PartialSharesGenerator<D, D2> {
    pub fn new(player_seeds: &[[u8; KEY_SIZE]], omit: usize) -> Self {
        ...
        Self {
            ...
            input_2: PartialShareGenerator::new(input_2_prgs, omit),
